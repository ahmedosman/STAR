...
def quaternions_all(p):
    ...
    angle = tf.reshape(tf.sqrt(tf.clip_by_value(tf.reduce_sum(tf.square(p), 2),
...
def global_rigid_transformation(rot_mat, J):
    ...
    :param rot_mat: batch_size x num_joints x 3 x 3 tensor of the
                    rotation matrices.
    :param J: batch_size x num_joints x 3 tensor of 3D joints positions
    :param kintree_table: a 2D array encoding the body kinematic tree.
                          First row is the index of the parent node,
                          second row is the index of a child node.
    ...
    Js = tf.cast(J, tf.float32) - tf.concat([tf.zeros([batch_size, 1, 3],
                                                      dtype=tf.float32),
                                             tf.gather(J, parent_nodes,
                                                       axis=1)], axis=1)
    Js = tf.expand_dims(Js, axis=-1)
    T = tf.concat([tf.cast(rot_mat, tf.float32), Js], axis=-1)
    T = tf.concat([T, tf.tile(tf.constant([[[[0.0, 0.0, 0.0, 1.0]]]],
                              dtype=tf.float32), [batch_size, num_joints,
                                                  1, 1])], axis=2)
    ...
    for i in range(1, J.shape[1]):
        results[i] = tf.einsum('ijk,ikl->ijl', results[kintree_table[0, i]],
                               T[i])
    results_global = tf.stack([results[i] for i in range(0, J.shape[1])],
                              axis=1)

    Jt = tf.einsum('ijkl,ijl->ijk', results_global,
                   tf.concat([J, tf.zeros([batch_size, num_joints, 1],
                                          dtype=tf.float32)], axis=-1))
    results = results_global - tf.concat([tf.zeros([batch_size,
                                                    num_joints, 4, 3],
                                                   dtype=tf.float32),
                                         tf.expand_dims(Jt, axis=-1)], axis=-1)
...
def verts_core(pose, v, J, weights, kintree_table):
    ...
    A_w = tf.einsum('ijkl,mj->imkl', A, weights)

    rest_shape_h = tf.concat([v, tf.ones([batch_size, 6890, 1],
                                         dtype=tf.float32)], axis=-1)
    v = A_w[:, :, :, 0]*tf.expand_dims(rest_shape_h[:, :, 0], axis=-1) + \
        A_w[:, :, :, 1]*tf.expand_dims(rest_shape_h[:, :, 1], axis=-1) + \
        A_w[:, :, :, 2]*tf.expand_dims(rest_shape_h[:, :, 2], axis=-1) + \
        A_w[:, :, :, 3]*tf.expand_dims(rest_shape_h[:, :, 3], axis=-1)
...
def tf_rodrigues(p):
    ...
    angles = tf.reshape(tf.sqrt(tf.clip_by_value(tf.reduce_sum(tf.square(p), 2),
                                                 tf.constant(1e-16, dtype=tf.float32),
                                                 tf.constant(1e16, dtype=tf.float32)),
                                name='angles'), [batch_size, num_joints, 1])
    norm_p = p / angles

    ppt = tf.einsum('ijkl,ijlm->ijkm', tf.expand_dims(norm_p, axis=-1),
    ...
    skewmat = tf.reshape(tf.concat([zrs, -rz, ry, rz, zrs,
                                    -rx, -ry, rx, zrs], axis=-1),
                         [batch_size, num_joints, 3, 3])
    cos_angles = tf.tile(tf.expand_dims(tf.cos(angles), axis=-1),
                         [1, 1, 3, 3])
    sin_angles = tf.tile(tf.expand_dims(tf.sin(angles), axis=-1),
                         [1, 1, 3, 3])

    R = tf.multiply(cos_angles,
                    tf.tile(tf.expand_dims(tf.expand_dims(tf.eye(3, 3,
                                                                 dtype=tf.float32),
                                                          axis=0), axis=0),
...
def lrotmin(p):
    ...
    I = tf.tile(tf.expand_dims(tf.expand_dims(tf.eye(3, 3, dtype=tf.float32),
                                              axis=0), axis=0),
...
class STAR(object):
    ...
    def __call__(self, pose, betas, trans):
        ...
        self.J_regressor = tf.constant(self.smpl_model['J_regressor'],
                                       dtype=tf.float32)
        self.posedirs = tf.constant(self.smpl_model['posedirs'],
                                    dtype=tf.float32)
        self.shapedirs = tf.constant(self.smpl_model['shapedirs'][:, :, :self.num_betas],
                                     dtype=tf.float32)
        ...
        v_shaped = tf.add(tf.einsum('ijk,lk->lij', self.shapedirs, betas),
                          tf_v_template)

        pose_feat = tf.concat([quaternions_all(tf.reshape(pose, (-1, 24, 3)))[:, 4:],
                               tf.expand_dims(betas[:, 1], axis=1)], axis=1)
